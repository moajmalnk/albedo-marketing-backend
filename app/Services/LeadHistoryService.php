...
/**
 * Builds a unified CRM timeline for a lead: audit rows, pipeline transitions,
 * and telecaller/sales activities.
 */
class LeadHistoryService
{
    ...
    public function timeline(Lead $lead, int $perSourceLimit = 250, int $mergeLimit = 500): array
    {
        ...
        $transitions = LeadStageTransition::query()
            ->with(['changedByUser.role'])
            ->where('lead_id', $leadId)
            ->orderByDesc('changed_at')
            ->limit($perSourceLimit)
            ->get();
        $transitionIds = $transitions->pluck('id');
        ...
        foreach ($audits as $log) {
            ...
            if ($log->entity_type === 'lead_stage_transition') {
                $auditedTransitionIds->push((int) $log->entity_id);
            }
        }

        foreach ($transitions as $transition) {
            if (! $auditedTransitionIds->contains((int) $transition->id)) {
                $rows->push($this->formatTransitionRow($transition, $stageLabels));
            }
        }
     ...
     * @return array<string, mixed>
     */
    private function formatTransitionRow(LeadStageTransition $transition, Collection $stageLabels): array
    {
        $occurredAt = $transition->changed_at instanceof Carbon
            ? $transition->changed_at->toIso8601String()
            : (string) $transition->changed_at;

        $new = $transition->toArray();
        [$title, $description] = $this->describeStageTransitionAudit($new, $stageLabels);
        ...
        return [
            ...
            'title' => $title,
            'description' => $description,
            'actor' => $this->serializeActor($transition->changedByUser),
            'ip' => null,
            'user_agent' => null,
            'old_values' => [],
            'new_values' => $new,
    ...
    private function describeAudit(string $action, array $old, array $new, Collection $stageLabels): array
    {
        return match ($action) {
            ...
            'lead.deleted' => ['Lead deleted', 'Lead record removed.'],
    ...
    private function describeLeadUpdated(array $old, array $new, Collection $stageLabels): array
    {
        ...
        foreach ($keys as $key) {
            ...
            if ($key === 'stage_id') {
                ...
                $readable[] = 'assignee: '.($before ?? '-').' -> '.($after ?? '-');
        ...
        $summary = $readable === [] ? 'Record touched with no visible field change.' : implode('; ', $readable);
